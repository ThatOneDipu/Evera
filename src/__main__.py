...
def main(version="devel", pkgdatadir="/app/share/evera", localedir="/app/share/locale"):
    # Make sure that X11 is the backend. Revert Wayland to XWayland.
    os.environ["GDK_BACKEND"] = "x11"
    # Suppress VLC Log
    os.environ["VLC_VERBOSE"] = "-1"

    parser = argparse.ArgumentParser(description=f"Evera v{version}")
    parser.add_argument("-p", "--pause", dest="p", type=int, default=0,
                        help="Add pause before launching Evera. [sec]")
    ...
    sys_info.append("--------------------------")
    sys_info_str = "\n".join(sys_info)
    logger.info(f"Evera v{version}\n{sys_info_str}")
    logger.info(f"[Args] {vars(args)}")

    # Make Evera folder if not exist
